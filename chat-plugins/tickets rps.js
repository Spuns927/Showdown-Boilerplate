	...
	onEnd(inactivity) {
		...
		if (gameResult === "pp") {
			...
			if (this.gameType === "bucks") {
				...
				Db("tickets").set(this.p1.userid, Db("tickets").get(this.p1.userid, 0) + 3);
				Db("tickets").set(this.p2.userid, Db("tickets").get(this.p2.userid, 0) + 3);
			}
...
exports.commands = {
	rps: {
		search: function (target, room, user) {
			...
			if (target && target === "tickets") {
				if (Db("tickets").get(user.userid, 0) >= 3) {
					gameType = "tickets";
					Db("tickets").set(user.userid, (Db("tickets").get(user.userid, 0) - 3));
				} else {
					return this.errorReply("You do not have enough tickets (3) to search for a game of Rock/Paper/Scissors for bucks.");
				}
			}
		...
		endsearch: function (target, room, user) {
			...
			updateSearches();
			if (Rooms.global.RPS.searches[user.userid] === "tickets") {
				Db("tickets").set(user.userid, Db("tickets").get(user.userid, 0) + 3);
			}
	...
	rpshelp: ["/rps search (tickets) - searches for a game of Rock/Paper/Scissors either for ladder points or for tickets.",
